...
contract WhimLandVault is IWhimLandVault, OwnableUpgradeable {
    ...
    function depositETH(OrderKey orderKey, uint256 ETHAmount) external payable onlyWhimLandOrderBook {
        require(msg.value >= ETHAmount, "HV: not match ETHAmount");
        ETHBalance[orderKey] += msg.value;
    }

    function withdrawETH(OrderKey orderKey, uint256 ETHAmount, address to) external onlyWhimLandOrderBook {
    ...
    function depositERC20(OrderKey orderKey, uint256 ERC20Amount, address currency, address from)
        external
        onlyWhimLandOrderBook
    {
    ...
    function withdrawERC20(OrderKey orderKey, uint256 ERC20Amount, address currency, address to)
        external
        onlyWhimLandOrderBook
    {
        ERC20Balance[orderKey] -= ERC20Amount;
    ...
    function editERC20(
        ...
        if (oldERC20Amount > newERC20Amount) {
            ...
            IERC20(currency).safeTransfer(orderMaker, oldERC20Amount - newERC20Amount);
        } else if (oldERC20Amount < newERC20Amount) {
            IERC20(currency).safeTransferFrom(orderMaker, address(this), newERC20Amount - oldERC20Amount);
            ERC20Balance[newOrderKey] = newERC20Amount;
        } else {
    ...
    function editNFT(OrderKey oldOrderKey, OrderKey newOrderKey) external onlyWhimLandOrderBook {
    ...
    function transferERC721(address from, address to, LibOrder.Asset calldata assets) external onlyWhimLandOrderBook {
        IERC721(assets.collectionAddr).safeTransferNFT(from, to, assets.tokenId);
    }

    function batchTransferERC721(address to, LibOrder.NFTInfo[] calldata assets) external onlyWhimLandOrderBook {
