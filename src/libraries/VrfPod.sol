...
contract VrfPod is Initializable, OwnableUpgradeable, VrfPodStorage {
    modifier onlyVrfManager() {
        require(
            msg.sender == vrfManager,
            "DappLinkVRF.onlyVrfManager can call this function"
        );
        _;
    }
    ...
    function getRandomWordsWithStatus(
        uint256 _requestId
    ) external view returns (bool fulfilled, uint256[] memory randomWords) {
        return (
            randomWordsMapping[_requestId].fulfilled,
            randomWordsMapping[_requestId].randomWords
        );
    }
