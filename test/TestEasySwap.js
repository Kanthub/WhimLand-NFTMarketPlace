...
describe("EasySwap Test", function () {
    ...
    describe.skip("should edit order successfully", async () => {
        it("should edit list order successfully, new price > old price", async () => {
            const now = parseInt(new Date() / 1000) + 100000
            const salt = 1;
            const nftAddress = testERC721.address;
            const tokenId = 1;
            ...
            await esDex.editOrder(orderHash, newOrder)
        ...
        it("should edit list order successfully, new price < old price", async () => {
            ...
            newOrderKeys = await esDex.callStatic.editOrder(orderHash, newOrder, { value: toBn("0.01") })
            expect(newOrderKeys[0]).to.not.equal(Byte32Zero)

            await expect(await esDex.editOrder(orderHash, newOrder, { value: toBn("0.01") }))
                .to.changeEtherBalances([owner, esVault], [toBn("0"), toBn("0")]);
        ...
        it("should edit bid order successfully, new price < old price", async () => {
            const now = parseInt(new Date() / 1000) + 100000
            const salt = 1;
            const nftAddress = testERC721.address;
            const tokenId = 0;
            ...
            await expect(await esDex.makeOrders(orders, { value: toBn("0.03") }))
                .to.changeEtherBalances([owner, esVault], [toBn("-0.01"), toBn("0.01")]);
            ...
            newOrderKeys = await esDex.callStatic.editOrder(orderHash, newOrder, { value: toBn("0.2") })
            expect(newOrderKeys[0]).to.not.equal(Byte32Zero)

            await expect(await esDex.editOrder(orderHash, newOrder, { value: toBn("0.2") }))
                .to.changeEtherBalances([owner, esVault], [toBn("0.005"), toBn("-0.005")]);
            const newOrderHash = await testLibOrder.getOrderHash(newOrder)
            ...
            expect(newETHBalance).to.equal(toBn("0.005"))

            oldETHBalance = await esVault.ETHBalance(orderHash);
            expect(oldETHBalance).to.equal(0)
        })

        it("should edit bid order successfully, new price < old price, new amount < old amount", async () => {
            const now = parseInt(new Date() / 1000) + 100000
            const salt = 1;
            const nftAddress = testERC721.address;
            const tokenId = 0;
            ...
            await expect(await esDex.makeOrders(orders, { value: toBn("0.05") }))
                .to.changeEtherBalances([owner, esVault], [toBn("-0.03"), toBn("0.03")]);
        ...
        it("should edit bid order successfully, new price < old price, new amount > old amount, new total price < old total price", async () => {
            const now = parseInt(new Date() / 1000) + 100000
            const salt = 1;
            const nftAddress = testERC721.address;
            const tokenId = 0;
            ...
            await expect(await esDex.editOrder(orderHash, newOrder))
                .to.changeEtherBalances([owner, esVault], [toBn("0.005"), toBn("-0.005")]);
            const newOrderHash = await testLibOrder.getOrderHash(newOrder)
            ...
            newETHBalance = await esVault.ETHBalance(newOrderHash);
            expect(newETHBalance).to.equal(toBn("0.025"))
        ...
        it("should edit bid order successfully, new price < old price, new amount > old amount, new total price > old total price", async () => {
            ...
            newOrder = {
                ...
                saleKind: SaleKind.FixedPriceForItem,
                maker: owner.address,
                nft: [tokenId, nftAddress, 10],
                price: toBn("0.005"),
                expiry: now,
                salt: salt,
            }

            newOrderKeys = await esDex.callStatic.editOrder(orderHash, newOrder, { value: toBn("0.3") })
            expect(newOrderKeys[0]).to.not.equal(Byte32Zero)

            await expect(await esDex.editOrder(orderHash, newOrder, { value: toBn("0.3") }))
                .to.changeEtherBalances([owner, esVault], [toBn("-0.02"), toBn("0.02")]);
            const newOrderHash = await testLibOrder.getOrderHash(newOrder)
            ...
            newETHBalance = await esVault.ETHBalance(newOrderHash);
            expect(newETHBalance).to.equal(toBn("0.05"))
        ...
        it("should edit bid order successfully, new price > old price, new amount > old amount", async () => {
            ...
            newOrderKeys = await esDex.callStatic.editOrder(orderHash, newOrder, { value: toBn("0.05") })
            expect(newOrderKeys[0]).to.not.equal(Byte32Zero)

            await expect(await esDex.editOrder(orderHash, newOrder, { value: toBn("0.05") }))
                .to.changeEtherBalances([owner, esVault], [toBn("-0.03"), toBn("0.03")]);
        ...
        it("should edit bid order successfully, new price > old price, new amount < old amount, new total price < old total price", async () => {
            ...
            await expect(await esDex.makeOrders(orders, { value: toBn("0.08") }))
                .to.changeEtherBalances([owner, esVault], [toBn("-0.06"), toBn("0.06")]);
        ...
        it("should edit bid order successfully, new price > old price, new amount < old amount, new total price > old total price", async () => {
            ...
            newOrderKeys = await esDex.callStatic.editOrder(orderHash, newOrder, { value: toBn("0.1") })
            expect(newOrderKeys[0]).to.not.equal(Byte32Zero)

            await expect(await esDex.editOrder(orderHash, newOrder, { value: toBn("0.1") }))
                .to.changeEtherBalances([owner, esVault], [toBn("-0.04"), toBn("0.04")]);
            ...
            newETHBalance = await esVault.ETHBalance(newOrderHash);
            expect(newETHBalance).to.equal(toBn("0.1"))
        ...
        it("should edit partly filled bid order successfully, new price > old price", async () => {
            orderHash = await perparePartlyFilledOrder();
            let now = parseInt(new Date() / 1000) + 10000000000
            let salt = 1;
            let nftAddress = testERC721.address;
            ...
            newOrderKeys = await esDex.connect(addr1).callStatic.editOrder(orderHash, newOrder, { value: toBn("0.5") })
            expect(newOrderKeys[0]).to.not.equal(Byte32Zero)

            await expect(await esDex.connect(addr1).editOrder(orderHash, newOrder, { value: toBn("0.8") }))
                .to.changeEtherBalances([addr1, esVault], [toBn("-0.05"), toBn("0.05")]);
            ...
            newETHBalance = await esVault.ETHBalance(newOrderHash);
            expect(newETHBalance).to.equal(toBn("0.08"))
        ...
        it("should edit partly filled bid order successfully, new price < old price", async () => {
            ...
            newOrderKeys = await esDex.connect(addr1).callStatic.editOrder(orderHash, newOrder)
            expect(newOrderKeys[0]).to.not.equal(Byte32Zero)
